...
%   (with multiple diffusion coefficients with a preference for one and
...
%diffusion coefficient over a less preferred diffusion coefficient. Once
%the particles have obtained a specific diffusion coefficient for movement,
%it will have a preference to remain in that initial state that is dictated
%by the user. This diffusion is consistent with a Markov Chain model to
%express a final diffusion coefficient for each molecule. 
...
Pix_Mag=((Pix_Size)/(Mag))*1000;%normalize pixel size to magnification, conver to nm
Lambda=input('Enter wavelength of light (nm) [d/f=720]:'); %enter light wavelength
if isempty(Lambda)
    Lambda=720;
end
...
D1_um_pix=(D1_um*1000^2)/((Pix_Mag)^2);
Prob_D1=(input('Enter probability of diffusion coefficent occurence (%):'))/100;
Pref_D1=(input('Enter preference to remain in diffusion state 1 once achieved (%):'))/100;

if (Prob_D1<0.5)
    error('The preferred diffusion coefficient must have a probability >50%');
end
if (Pref_D1<0.5)
    error('The preferred diffusion coefficient must have a preference >50%');
end

D2_um=input('Enter less preferred diffusion coefficient (um^2/s):');
D2_um_pix=(D2_um*1000^2)/((Pix_Mag)^2);
Prob_D2=1-Prob_D1;
disp(['Default probability of diffusion coefficient 2 will be ' num2str(Prob_D2*100) '%']);
Pref_D2=(input('Enter preference to remain in diffusion state 2 once achieved (%):'))/100;

%generate the initial diffusion coefficient states for all the molecules 
diffusionprobdata=rand(1,Molecules);
diffusiondata=zeros(1,Molecules);

for u=1:Molecules
    if diffusionprobdata(u)<=Prob_D1
        diffusiondata(u)=D1_um_pix;
    else
    diffusiondata(u)=D2_um_pix;
    end
end
...
%generating a new image consistent with the sqrt(MSD) 
for m=1:Num_Frames
 
%generate a new noise background image for each iteration
Image.Output=normrnd(mB,sB,[w,h]);

%generate a preference diffusion vector for the data to remain in that
%specific diffusion coefficient
 prefdiffdata=rand(1,Molecules);
 prefdiffdata=cat(1,prefdiffdata,diffusiondata);
 
    
    for p=1:Molecules
        
%Markov Chain Model for preference for the diffusion state 
    if diffusiondata(p)==D1_um_pix
        if prefdiffdata(1,p)<Pref_D1
            prefdiffdata(2,p)=D1_um_pix;
        else
            prefdiffdata(2,p)=D2_um_pix;
        end
    end
     
    if diffusiondata(p)==D2_um_pix
        if prefdiffdata(1,p)<Pref_D2
            prefdiffdata(2,p)=D2_um_pix;
        else
            prefdiffdata(2,p)=D1_um_pix;
        end
    ...
    %reset the diffusion matrix for the next iteration
    diffusiondata=prefdiffdata(2,:);

    MSD=4*diffusiondata.*t; %calculate the expressed mean square displacement
    sqrtMSD=sqrt(MSD);
   
    diffusionlabel=diffusiondata;
    diffusionlabel(diffusionlabel==D1_um_pix)=D1_um;
    diffusionlabel(diffusionlabel==D2_um_pix)=D2_um;
    
    data(:,7)=diffusionlabel'; %generate a column of the data matrix to display the expressed MSD
    
    %generate a radius of displacement based off of a normrnd distribution
    %for each separate MSD
    r=normrnd(0,sqrtMSD(p));  
